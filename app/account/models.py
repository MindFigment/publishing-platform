...
class Profile(models.Model):
    ...
    photo = models.ImageField(
        upload_to="users/%Y/%m/%d/", default="users/default/default_avatar.jpeg"
    )
    ...
    def get_new_followers_count(self):
        return (
            apps.get_model("blogs", "FollowRelationship")
            .objects.filter(blog__author__user=self.user)
            .filter(seen=False)
            .count()
        )
    ...
    def get_old_and_new_followers(self):
        ...
        new_followers = followers.filter(seen=False).annotate(old=F("seen"))
        old_followers = followers.filter(seen=True).annotate(old=F("seen"))
